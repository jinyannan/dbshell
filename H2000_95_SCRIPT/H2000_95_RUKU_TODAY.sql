...
then
     psql -d h2000 -f /data/xloader/db_shell/today_shell/ENTRY_PROC_LOG_GPTMP_TODAY.sql
     if [ $? != 0 ]
     then
         exit 1
     fi
...
then
     psql -d h2000 -f /data/xloader/db_shell/today_shell/ENTRY_QUERY_GPTMP_TODAY.sql
     if [ $? != 0 ]
     then
         exit 1
     fi
